...
exportApp <- function(teamName = "", dbName = "data", logAsJson = TRUE) {
  ...
  shiny::shinyApp(ui = exportAppUi, server = exportAppServer)
}

#' Export app UI
#'
#' @returns UI function for exportApp
#' @keywords internal
exportAppUi <- function() {
  shiny::navbarPage(
    id = "navbarpage",
...
#'
#' @returns Server function for exportApp
#' @keywords internal
exportAppServer <- function(input, output, session) {

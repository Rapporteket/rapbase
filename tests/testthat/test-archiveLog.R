...
test_that("error is provided when log files does not exixt", {
  expect_error(archiveLog(
    archivePath = tempdir, logPath = tempdir,
...
test_that("an none-existing oversized log returns character()", {
  expect_equal(logsOverSize(
    logPath = tempdir, overSize = 1,
    pattern = "not_existing_at_all.csv"
  ), character())
})

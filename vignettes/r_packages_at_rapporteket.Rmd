...
vignette: >
  %\VignetteIndexEntry{Introducing R packages at Rapporteket}
  %\VignetteEngine{knitr::rmarkdown}
  %\VignetteEncoding{UTF-8}
---

Hoepfully, use of R packages will make both development and system meintenance
at Rappporteket easier. However, this calls for some changes of both routines
and configuration/set-up.
...
## User privileges
Intallation of packages must also work automated. In a manual approach
...
installed in the users home directory and will thus only be available to that
praticular user.
...
have to use a proxy server which have to be defined:

```r
Sys.setenv(http_proxy="http://www-proxy-rn.helsenord.no:8080")
```
...
```r
library(httr)
set_config(use_proxy(url="172.29.3.232", port=8080))
```

The actual installation can then be performed using the _devtools_ package,
the following being relevant for a TEST or QA environment:


```r
devtools::install_github("Rapporteket/rapbase", ref = "rel", args = c("--clean"))
```

For a PRODUCTION environment the following should be used:


```r
devtools::install_github("Rapporteket/rapbase", ref = "master", args = c("--clean"))
```

## Troubleshooting
Things may go wrong, awfully so. Misery collected:

### Proxy and TLS
Observed: packages (primarily our own) will not intstall because they depend on
other CRAN packages that cannot be installed.

Plausible cause: network communication with (CRAN) https mirrors fails,
time-out or stops due to internal proxies.

Solution: manually install dependency packages and selecting http mirrors
which may be an (last) option in the list of https mirrors.
